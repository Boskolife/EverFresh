...
function initBeforeAfterSlider() {
  const containers = document.querySelectorAll('.positioning__item-images');
  ...
  containers.forEach((container, index) => {
    ...
    const afterImage = container.querySelector(
      '.positioning__image.after-img',
    );

    if (!sliderWrapper || !slider || !beforeImage || !afterImage) return;

    const clipTarget = beforeImage.querySelector('img');
    if (!clipTarget) return;

    if (!container.id) {
      container.id = `before-after-slider-${index + 1}`;
    }
